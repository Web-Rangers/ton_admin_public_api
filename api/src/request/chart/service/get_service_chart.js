...
async function get_service_chart(service_name,time_period,time_value) {
    ...
    let [rows] = await db_connection.connection.query(`SELECT ROUND((UNIX_TIMESTAMP() - time)/(${time[0]})) as date,avg(latency)as value from service_ping WHERE service_name='${service_name}' and time >= ${now} group by date;`)
    return rows
}
