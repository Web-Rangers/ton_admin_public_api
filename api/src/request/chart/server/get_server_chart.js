...
async function get_server_chart(server_ip,server_port,time_period,time_value) {
    ...
    let [rows] = await db_connection.connection.query(`SELECT ROUND((UNIX_TIMESTAMP() - time)/(${time[0]})) as date,avg(latency)as value from server_ping WHERE server_ip='${server_ip}' and server_port = ${server_port} and time >= ${time_above} group by date;`)
    return rows
}
