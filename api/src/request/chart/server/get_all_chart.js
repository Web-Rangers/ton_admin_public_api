...
async function get_servers_chart(time_period,time_value) {
    ...
    let [rows] = await db_connection.connection.query(`SELECT ROUND((UNIX_TIMESTAMP() - time)/(${time[0]})) as date,avg(latency)as value from server_ping WHERE time >= ${time_above} and status_code=1 group by date;`)
    return rows
}
