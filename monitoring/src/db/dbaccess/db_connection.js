...
function connect(){
    ...
    class DbConnection{
        constructor(){
            ...
            this.connection.execute("CREATE TABLE IF NOT EXISTS status (electionId BIGINT,totalValidators INT,onlineValidators INT,startValidation BIGINT,endValidation BIGINT,startNextElection BIGINT,endElection BIGINT,last_block BIGINT,eth_bridge BOOLEAN NOT NULL,bsc_bridge BOOLEAN NOT NULL,tpsAvg VARCHAR(30),id MEDIUMINT NOT NULL AUTO_INCREMENT,PRIMARY KEY (id));")
            this.connection.execute("CREATE TABLE IF NOT EXISTS status_offers (approvedPercent INT,isPassed BOOLEAN,endTime INT,roundsRemaining INT);")
            ...
            this.connection.execute("CREATE TABLE IF NOT EXISTS status_transactions (from_ VARCHAR(200),hash VARCHAR(200),to_ VARCHAR(200),type VARCHAR(20),direction VARCHAR(20),block_height VARCHAR(200),value VARCHAR(1000),message VARCHAR(200),time BIGINT,PRIMARY KEY (hash));")
